...
**OB-B — business debt exclusion + rental offset (commit 2068645).** Extended ObligationResolver: `compute_business_debt` (EXCLUDE only if business-paid ≥ business_debt_exclusion_months=12 AND no 30-day delinquency, B3-3.4-02; else INCLUDE + docs_needed — is_business_paying/months_business_paid default False/0 so debt is included until the UW supplies 12mo cancelled-check/bank evidence); `compute_rental_offset` (net = rental_net_monthly − pitia_monthly, B3-3.1-08; ≥0 positive offset not a DTI obligation, <0 shortfall added; both inputs default 0 → not_applicable since RA-4G isn't bundle-wired + per-property PITIA absent). Routes business_debt + rental_property. 1 catalogue row seeded (business_debt_exclusion_months=12, B3-3.4-02; refined from B3-6-05 in 2f59551). HELOC was already complete in OB-A (no change). agency_guidelines 97→98 (Fannie 75). Reused RA-4G net rental + INC-F alimony; no logic rebuilt; graceful missing-input handling per spec.
...
## EX-A + EX-B — exception framework + compensating factors (UW OS) — 2026-06-24

Doc update batched across EX-A + EX-B (per user). Both ADVISORY foundation passes on the EX-A diagnostic finding: override CAPTURE already exists (loan_actions + decision_outputs.human_* + pattern_detector→policy_proposals + agent_learnings/PROMPT-L); what was missing is a structured exception + compensating-factors model. EX builds ON that, does not duplicate it.

**EX-A — exception framework (commit 27bfdbc).** New `core/exceptions/exception_engine.py:ExceptionEngine` (sync + DB-less, rule_loader.SAFE_DEFAULTS fallback): `evaluate_exception_eligibility` runs three gates — agency floor ABSOLUTE (below → never) → overlay-breach tolerance (catalogue max %) → compensating-factors-required; `classify_exception_type` (signal→type). RULE 11 on every return. New RDS tables `loan_exceptions` (request→review→grant lifecycle; FK decision_outputs + loan_actions; breach_pct/below_agency_floor/compensating_factors JSONB; tenant_id_rls generated col) + `compensating_factors` (per-factor; FK loan_exceptions) — `create_exception_tables.sql`. 4 catalogue rows (Fannie B3-2-02). Wired into approval_routing via runner `exception_rules` bundle key → advisory `output_payload.exception_analysis`. EX-A does NOT write the tables (EX-C). proposed_outcome UNCHANGED (Gap f). agency_guidelines 98→102. Caught+fixed pre-merge: approval_routing used latest_object without import (NameError surfaced by the offline persona test) → added import; stopped the stale broken-code eval; re-verified.

**EX-B — compensating factors engine (commit cec8aad).** `core/exceptions/compensating_factors_engine.py:CompensatingFactorsEngine` (sync + DB-less): detects 6 factors from REAL entity_states data — substantial_reserves (total_liquid_assets/piti_monthly), low_ltv (vs LTV max), excellent_credit (mid_credit_score − Minimum Credit Score), long_employment (months from borrower.employment.period_start), limited_debt (monthly_obligations/qualifying_monthly), large_down_payment (100−ltv); scores strong(3)/moderate(2)/weak(1) → exception_score → recommended_approval_level (senior/manager/uw/insufficient). payment_shock + residual_income → not_applicable + missing_inputs (no baseline/VA-residual data). RULE 11 on every detect_*. 6 catalogue factor-bar thresholds (B3-2-02): substantial_reserves_months=6, exceptional_reserves_months=12, low_ltv_factor_max_pct=75, excellent_credit_delta_pts=60, long_employment_months=60, minimal_debt_obligations_max_pct=10. agency_guidelines 102→108 (Fannie 85).

**Wiring (EX-B, one shared bundle key — no runner change):** EXCEPTION_RULE_KEYS extended with the 6 factor thresholds + minimum_reserves_months baseline, so the existing approval_routing→exception_rules runner branch loads them all. Enricher `_attach_compensating_factor_inputs` surfaces the factor inputs (credit/ltv/piti/liquid/obligations/qualifying + employment.period_start + Minimum Credit Score floor via rule_loader) to the approval_routing bundle as `cf_inputs` (INC-E _attach_income_entity precedent). approval_routing runs detect_all(cf_inputs) → feeds present factors into the EX-A ExceptionEngine (was []) → `output_payload.compensating_factors_analysis` + enriched exception_analysis (score + recommended_approval_level). ADVISORY; proposed_outcome UNCHANGED.
...
## TR-D — repurchase defense report generator — 2026-06-24

**TR-D (commit d298631).** SHIPPED. A pure READ-ONLY assembler that compiles an 8-section repurchase-defense package from existing data sources — **no new tables / catalogue / outcome changes**, nothing written. `core/audit/reports/repurchase_defense.py:generate_repurchase_defense_report(conn, application_id, tenant_id)` returns a JSON report + rendered HTML (PDF deferred to a follow-up slice). Exported from `core/audit/reports/__init__.py` alongside `generate_exception_register`.

**The 8 sections (each → its real source table):** (1) Loan Summary ← `entity_states`; (2) Decision Summary ← `decision_outputs` (current = MAX version per decision_id, `DISTINCT ON`); (3) Policy Trace ← `decision_trace.policy_trace.rule_disclosures` (federal/agency/overlay/applied + citation); (4) Evidence Quality per Document ← `document_index` (confidence + extraction method per doc); (5) AUS Reconciliation ← approval_routing payload + `aus_responses`; (6) Exception Status ← `loan_exceptions` + `compensating_factors`; (7) Adverse Action Notice ← `adverse_action_notices`; (8) Compliance Attestation ← `compliance_check` payload (ATR/QM).

**RULE 11 throughout:** every section carries `data_source` + `missing_inputs`, and **N/A is distinguished from missing** — meridian has no DU/LP so AUS Reconciliation is correctly `not_applicable`, not "incomplete". `context_snapshot` is read as the flat `output_payload` with a snapshot fallback (`.get("output_payload", snap)`) and the latest-payload helper walks versions newest-first so a best-effort-empty version doesn't blank a section.

**Verification.** Functional on **APP-MRID-SC08**: 7/8 sections complete + AUS correctly N/A (Compliance Attestation atr=True, 8/8, QM=NON_QM); HTML renders + closes (~12KB), JSON-serializable. 9 pure render/format unit tests (`tests/test_repurchase_defense.py` — structure, N/A handling, safe money/pct, real rule-disclosure keys; the async generator is DB-bound, covered functionally). Full suite **13 failed / 611 passed** (the 13 are the established pre-existing env/DB baseline; zero new). Meridian eval **16/16** — read-only, no decision-path change, unaffected by construction. Remaining: PDF rendering follow-up.

## CI-A — change impact simulator — 2026-06-25

**CI-A (commit 6ee0d7e).** SHIPPED. `core/intelligence/change_impact_simulator.py:ChangeImpactSimulator.simulate(conn, tenant_id, rule_name, current_value, hypothetical_value)` — a READ-ONLY "what-if" over the recorded pipeline: "if we moved this overlay rule, which apps flip and how many dollars move?" **No catalogue writes, no decision writes** (same posture as TR-D). NOT an engine re-run — the full 14-persona dry-run is CI-B (future). New `core/intelligence/` package + API `api/accord/intelligence.py` (POST `/api/accord/intelligence/simulate-impact` + GET `/simulatable-rules`), router registered in `api/accord/__init__.py` (16 routers).

**Approach (delta short-circuit + binding-constraint cross-check).** 3 simulatable overlay levers, each → (entity field, upstream persona, gate dir): `credit_floor`→(mid_credit_score, credit_assessment, gte), `dti_back_max`→(dti_back, dti_calculation, lte), `ltv_max_purchase`→(ltv, ltv_assessment, lte). Per app: re-evaluate ONLY that gate vs the hypothetical threshold; flip the controlled persona ONLY if the field gate flips (a non-threshold block — bankruptcy/thin-file — is left intact); then re-reduce the recorded upstream outcomes (one persona swapped) to the simulated underwriting outcome via the real boundary (any block→block; else any escalate→escalate; else recommend).

**Two spec corrections caught against live data (the spec's STEP-1/STEP-2 were schematic):** (1) blocking constraint read from the authoritative `decision_outputs.upstream_decisions` COLUMN — `output_payload.signals` is EMPTY in production, and following the spec verbatim would make every app look sole-binding → false flips everywhere. The column `reduce()` reproduces all **16/16** recorded meridian outcomes (validated). (2) column-name fixes: `outcome`/`decision_id` (not `proposed_outcome`/`persona_id`).

**Correctness guarantee — flip only if the rule is the SOLE binding constraint.** Re-reducing the full upstream set excludes multi-constraint apps automatically; a would-be flip masked by another blocker is reported as `shadowed` (dollars NOT counted), not dropped. RULE 11: NULL field → `missing_inputs`, never assumed; every result + the top-level dict carry `data_source`; top level carries a plain-language `honesty_caveat`.

**Proven on meridian (16 apps):** loosening credit/dti/ltv → **$0 true unblocks** (every blocked app is also `product_eligibility`-blocked — honest, not oversold); the **SC01 shadowing guard** correctly excludes **$1.99M** (5 apps clear DTI but stay blocked on fraud/product); tightening DTI 43→38 → **3 apps / $1.15M** newly at risk (SC07/SC14/SC16 at dti≈42); SC03 NULL dti_back → skipped via RULE 11; unknown rule → error + missing_inputs.

**Verification.** **18/18** unit tests (`tests/test_change_impact_simulator.py` — pure gate/reduce/persona-flip/classify + 7 fake-conn integration cases: sole-constraint unblock, shadowed-not-counted, RULE 11 NULL, tighten→new-block, error path, provenance). Full suite **13 failed / 629 passed** (629 = 611 + 18; the 13 are the established pre-existing env/DB baseline; zero new). Meridian eval **16/16 by construction** — CI-A is read-only, no decision-path change; the live-RDS eval hung this session (sequential + parallel, network-degraded — gap g) so a confirmatory run was skipped per the documented gap. Remaining: CI-B (full 14-persona dry-run for cascade fidelity); optional loan_type scoping of dti_back_max (43 conventional vs 50 fha).

## CI-B — historical decision replay + eval timeout fix — 2026-06-25

**CI-B (commit 0f6245c).** SHIPPED. `core/intelligence/decision_replay.py` — replay any recorded decision against a different `tenant_rules` version, READ-ONLY (no writes to decision_outputs/persona_bundles). `replay_decision(conn, application_id, tenant_id, target_rule_version_id)` + `replay_all_decisions(conn, tenant_id, target_rule_version_id)`. Reuses CI-A's `_reduce_outcome`/`_normalize_upstream` from `change_impact_simulator` (shared intelligence primitives).

**Diagnostic finding — most of the machinery already existed:** `ThresholdResolver(conn, tenant_id, rule_version_id)` is ALREADY version-parametric (the cron path scores rate-locked loans under their pinned version via `_resolve_rule_version`: pin → pipeline-cutoff grandfather → current active); `decision_outputs.rule_version_id` is stamped on every row. CI-B is mostly wiring + diffing. Approach: read recorded outcome + `rule_version_id` (decision_outputs), frozen upstream outcomes (`persona_bundles.upstream_snapshot`) + gate values (entity_states), re-resolve credit/dti/ltv thresholds at original-vs-target version, re-evaluate ONLY those gates, swap affected personas into the frozen upstream set, and re-reduce to the underwriting outcome. `reduce()` reproduces all 16 recorded outcomes → `fidelity_failures=0`.

**Spec corrections (verified vs live schema):** (1) `tenant_rules` stores rules in a `rules` JSONB (`{credit:{min_score},dti:{back_max},ltv:{max},...}`), NOT `rule_name`/`rule_value` columns — the seed writes a `rules` blob. (2) `ThresholdResolver.resolve(threshold_field, default_value, rule_version_id=)` takes a DOTTED field (`credit.min_score`/`dti.back_max`/`ltv.max`) + a required default, returns a `ThresholdResolution` (`.value`). (3) shadow-safe outcome via `upstream_snapshot` re-reduction instead of the spec's 3-gate-only `simulated_outcome` (which would reintroduce the CI-A false-flip bug). (4) v2 deltas: spec proposed `dti 43→45` (looser) but nothing on meridian sits in (43,45] → no-op; used `43→40` (stricter) for a real flip.

**Synthetic v2 (`scripts/compliance/seed_ci_b_v2_rules.py`).** Meridian had only v1, so nothing to replay against. Seeds `tenant_rules` v2 (rule_version_id `8718862a-…`, read from DB never hardcoded) as **`status='draft'` — NOT active**, so the live path still resolves v1 and decisions + 16/16 are UNCHANGED, while replay targets v2 by id (`_get_tenant_rules` reads any version by id, no status filter). Deltas: `credit.min_score` 640→680, `dti.back_max` 43→40 (both stricter). Idempotent (refresh-in-place). NOTE: this writes a draft row to the live meridian `tenant_rules` (intended per spec) — affects no decision since only active v1 is resolved.

**Eval timeout fix (gap g — CLOSED).** The eval was ALREADY concurrent (RA-P0-B/IN-B — `--concurrency=N` default 4); gap g's "sequential ~165s" was stale. The residual hang was an UNBOUNDED await on degraded RDS — a single stuck `_process_one` hung the whole `asyncio.gather`. Fixed with `asyncio.wait_for(timeout=SCENARIO_TIMEOUT)` (default 30s, env-tunable) around `_process_one` in `scripts/evaluate_meridian_scenarios.py`: a stuck decision → bounded `TIMEOUT` error, run continues.

**Proven on meridian — v1 → v2 (draft):** 3 of 16 decisions flip block (SC07/SC14/SC16, dti 42.0 fails the v2 40-cap) = **$1,152,500** — independently cross-validates CI-A's tighten-DTI finding via the replay path. SC01/SC03 stay block (shadow-safe); SC03 NULL dti_back → `missing_inputs` (RULE 11). `fidelity_failures=0`.

**Verification.** **8/8** unit tests (`tests/test_decision_replay.py` — cross-version flip, shadow-safe already-blocked, gate-clears-but-other-persona-blocks, RULE 11 NULL, no-decision/no-bundle errors, provenance, bulk rollup; the eval script can't be imported per ARCHITECTURE so the timeout guard is verified functionally). Full suite **13 failed / 650 passed** (650 = 642 + 8; the 13 are the established pre-existing env/DB baseline; zero new). Meridian eval **16/16 PASS** (exit 0, confirmed WITH the timeout guard active at `SCENARIO_TIMEOUT=1` — no false timeouts; v2 draft → decision path unchanged). Intelligence phase CI-A + CI-B complete; remaining: full 14-persona cascade replay (future), optional loan_type scoping.

## SC-B + SC-C — scenario infrastructure (`core/scenarios/`) — 2026-06-25

**SC-B (commit cb36b96) + SC-C (commit 6860584).** New `core/scenarios/` package — the typed scenario library + the tenant-agnostic engine that runs it. Read-only over the decision path (runs EXISTING apps, never fabricates loans).

**SC-B — Scenario library.** `base.py`: `Scenario` + `ScenarioCondition` dataclasses. A Scenario keeps BOTH `expected_key_decision`/`expected_outcome` (the per-persona decision the 16/16 eval checks) AND `underwriting_outcome` (the loan aggregate) — they differ (e.g. SC16 key closing_readiness=escalate vs loan recommend); plus provenance (`conditions`/`notify_role`/`explanation`) + RULE 11 (`data_source`/`missing_inputs`); helpers `reserve_months`/`is_multi_block`/`demo_talking_points`. `meridian.py`: the 16 scenarios as the single source of truth, replacing the loose `EXPECTED_OUTCOMES` + `SCENARIO_NOTES` dicts. Built from LIVE data (cross-checked **0 mismatches**); conditions computed from real values vs the meridian overlays (credit 660 / dti 43 / ltv 95) — factual breaches only (SC03 NULL dti → `missing_inputs`, no fabricated condition; SC08 multi-block; SC16 clean). **Two spec drafts corrected:** the spec's `expected_outcome`s were the underwriting aggregate not the per-key-decision `EXPECTED_OUTCOMES` (8 wrong) and its uniform piti/qual/assets/oblig were placeholders — both fixed against live data. 14 library tests.

**SC-C — `ScenarioRunner` (tenant-agnostic engine).** `runner.py:ScenarioRunner(database_url, tenant_id, concurrency, timeout)`, extracted from the meridian eval so the 16/16 gate AND `generate_scenarios.py` drive the SAME path. `execute()` runs every (wave, decision) via the REAL `PersonaRunner._process_one` (one decision/call, written not returned) — apps concurrent under a semaphore, each bounded by `SCENARIO_TIMEOUT`; `verify_one()` PASS/FAIL on the KEY decision (the eval's criterion), aggregate via the reused CI-A `_reduce_outcome` (no dup); `report_one()` REPORTED for library-less tenants (no invented PASS/FAIL); `_build_summary()` counts/$/pass-rate. **Spec correction:** the spec's `run_one(conn, app, tenant)` returning `upstream_decisions` was fictional — `_process_one` is a `PersonaRunner` method writing one decision; engine rebuilt against the real API (wave-ordered drive + read-back). `scripts/generate_scenarios.py` CLI: `--tenant` (any of the 7: meridian 16 / summit 49 / atlas-heartland-pacific 50 / demo 8696 / default 1 — only meridian has a library), `--concurrency`/`--seq`/`--direct`/`--scenario`. `evaluate_meridian_scenarios.py` delegates its run loop to `ScenarioRunner.execute()` but KEEPS its ordered `EXPECTED_OUTCOMES` + `SCENARIO_NOTES` + verification prints/query verbatim so 16/16 output is byte-identical (rebuilding the dict from the library would reorder the lines).

**Verification.** 14 SC-B + 9 SC-C unit tests (`tests/test_scenario_library.py`, `tests/test_scenario_runner.py` — fake conn + stubbed engine: verify PASS/FAIL, library-less report, run_all pass-rate, summary). Full suite **13 failed / 673 passed** (673 = 650 +14 +9; the 13 are the established baseline; zero new). `py_compile` clean on all 3 touched scripts. **16/16 byte-identical by construction** (same prints/dicts/query/order; only the run loop relocated with the identical `_process_one` call that ran green earlier this session) — live re-run + byte-diff skipped (network-degraded, gap g — eval hung at every attempt this session). Honest flags: only meridian has a library (others REPORTED, no fabrication); eval keeps ordered dicts for byte-identical (full eval→library migration deferred as cosmetic). SC-B + SC-C = complete scenario infrastructure.


## HA-A → HA-F — high availability + incident runbook — 2026-06-27

**HA phase (commit 7917ce4).** SHIPPED. Six artifacts — infra specs + thin decision-path-inert code — under `docs/ha/` + `core/infra/` + `scripts/ops/`. AWS account is real and reachable (621646470377, ambient ~/.aws creds) but the running platform has NO AWS env wiring (no SQS_QUEUE_URL, no S3_BUCKET, single-pool RDS); so Multi-AZ/ECS/CloudWatch are config-to-apply, while the code half ships now and runs in the graceful-no-AWS mode. All read-only/additive → **16/16 by construction**.

**HA-A — `docs/ha/HA-A-MULTI-AZ-RDS.md`.** Multi-AZ RDS spec (db.r6g.xlarge, multi_az + <2min auto-failover, async read replica, max_connections=500, pg_stat_statements, 7-day backup + 35-day PITR, Sun 03:00-04:00 maint, 60s enhanced monitoring) + Terraform snippet + writer/reader endpoint change. **In-flight decisions on failover → runner retries:** cites the existing `runner.py:258-290` one-shot pool-reset+retry (`_looks_like_conn_error` → `_reset_pools` → re-run `_process_one`) + SQS redelivery; decisions idempotent per (application_id, decision_id), so no manual intervention for an automatic failover.

**HA-B — `docs/ha/HA-B-ECS-AUTOSCALING.md` + `api/accord/health.py`.** ECS auto-scaling (CPU 70% target tracking, +2/2min, -1/10min, min 2 / max 10), ALB health check (30s, 3 failures, 30s dereg delay), rolling deploy (50/200 + rollback). NEW `GET /api/accord/health/deep` (JWT-gated): db_latency_ms (real SELECT 1 round-trip) + sqs_configured + s3_configured + test_count (from `docs/ha/test_count.txt`) + last_decision_timestamp + status (degraded if DB down); never raises. **Spec correction:** kept the cheap public `/api/accord/health` ({status:ok}) for the ALB probe — a 30s liveness check must not hit the DB; deep component checks live on the separate authenticated route.

**HA-C — `core/infra/circuit_breaker.py` + `docs/ha/HA-C-CIRCUIT-BREAKER.md`.** Pure-Python circuit breaker (no deps): CLOSED → OPEN (5 failures/60s) → HALF_OPEN (30s cooldown, one trial) → CLOSED. Thread-safe, INJECTABLE clock (now_fn) for deterministic tests. `get_breaker(name)` process-wide singleton; `snapshot()` = RULE 11 provenance (state + failure_count + retry_after_sec). Documented graceful degradation per dependency — claude_api → regex/pdfplumber (RA-EX-F), s3_upload → local/no-op (RA-P0-A), sqs_send → synchronous inline (IN-A); every degraded path already works. Built + unit-ready; wrapping the actual call sites is a follow-up.

**HA-D — `docs/ha/HA-D-BACKUP-RECOVERY.md` + `scripts/ops/verify_backup.py`.** RTO 30min (Multi-AZ) / 2hr (PITR); RPO 5min / 1hr. Verifier checks RDS snapshot < 24h + S3 versioning enabled + tenant_rules export < 7 days; graceful-no-AWS: returns `unknown` (never a false `healthy`) when AWS unconfigured. Ran live against the real account — honestly reported `accord-prod`/`accord-docs` not yet provisioned (the true foundation state). Catalogue needs no data backup (re-seedable from `scripts/compliance/seed_*.py` in git); tenant_rules IS backed up on activation.

**HA-E — `docs/ha/HA-E-OBSERVABILITY.md` + `core/infra/pipeline_monitor.py`.** NEW `emit_decision_metrics(outcome, latency_ms, persona_count, tenant_id, persona_failures, connection_pool_size)` on the IN-C CloudWatchMetricSink: DecisionLatencyMs + PersonaFailures (namespace Accord/Decisions, dims tenant_id+outcome) + ConnectionPoolSize (Accord/DB). Best-effort, no-op without AWS. Alarms documented: watermark > 900s page (IN-C), DLQ > 10 alert, latency > 5000ms warn, PersonaFailures > 0 page. Structured-JSON log shapes + 4-row dashboard layout (described, not built).

**HA-F — `docs/ha/HA-F-RUNBOOK.md`.** 5 runbooks (RDB-001 RDS connection failure, RDB-002 stalled pipeline >15min, RDB-003 Claude API unavailable, RDB-004 wrong decision output, RDB-005 fair-lending alert) with REAL endpoint paths (`/infra/sqs-status`, `/infra/pipeline-health`, `/health/deep`, `/audit/hmda/overlay-disparity`, `/audit/fair-lending/self-test`) and the REAL `PersonaRunner(DATABASE_URL).run_all_waves(batch_size, tenant_id)` signature (no per-application arg). SEV-1/2/3 tiers (5-min/30-min/next-day SLA) + L1-L4 escalation.

**Verification.** Circuit-breaker transitions (CLOSED→OPEN→HALF_OPEN→CLOSED + half-open-reopen) verified with injectable clock; `/health/deep` registered; `emit_decision_metrics` emits; `verify_backup.py` ran live. 11 files, +1049/-2. **16/16 by construction** (all decision-path-inert). One pre-existing unrelated test failure (`test_health_returns_ok`: root `/health` asserts agents "13" vs the now-15 routers) — confirmed by stash that it fails WITHOUT the HA changes; not introduced here.

## DOC-A → DOC-D — legal/compliance document templates — 2026-06-27

**DOC phase (commit ffe1c74).** SHIPPED. Four Markdown TEMPLATES in `docs/` (no Python, no DB, no tests), grounded in REAL Accord facts (actual commit hashes, endpoint paths, persona names, risk tiers, catalogue counts, audit findings); each marked "TEMPLATE — requires legal review before use"; lender blanks as [BRACKETS]. 453 insertions. (Documented here for completeness — preceded the HA phase.)

**DOC-A `LENDER_AGREEMENT_TEMPLATE.md`** — 11-section Accord Platform Agreement (parties, advisory platform desc/14 personas, decision scope+limits, DPA, fair-lending, SR 11-7, liability, IP, term, governing law, signatures); processor/controller split; 1,215-test baseline. **DOC-B `MODEL_RISK_MANAGEMENT_POLICY.md`** — SR 11-7: inventory from MODEL_REGISTRY (4 high/7 med/3 low, mode/owner/wave from live runner config), tier validation cadence (annual/biennial/triennial), MR-B PSI monitoring with live readings (credit 0.012/dti 0.174/ltv 3.35 significant), change control + incident response. **DOC-C `FAIR_LENDING_POLICY.md`** — ECOA/HMDA/FHA: demographics-never-in-decision invariant, QA-A 8 proxy pairs (CI), CM-G/CM-D/CM-F/CF-B/CF-A, honest insufficient_data note for the meridian fixture. **DOC-D `DATA_RETENTION_PRIVACY_POLICY.md`** — GLBA/CCPA: PII_FIELDS, controller/processor roles, retention table (7yr app/decisions, 3yr HMDA LAR per 12 CFR 1003.5, 5yr model snapshots), RBAC + 58 RLS/28 tables WITH the QA-C `edms_admin bypassrls=true` finding disclosed (remediation pending), breach notification (72h target), state provisions (CA/NY/VA/CO).

## PERF-A → PERF-C — load testing + DB optimization + caching — FINAL PHASE — 2026-06-27

**PERF phase (commit 76128b6).** SHIPPED. The final build phase (125/125). Three artifacts under `scripts/perf/` + `docs/perf/` + `core/infra/`. All read-only / standalone / decision-path-inert → **16/16 by construction**. 6 files, +710.

**PERF-A — `scripts/perf/load_test.py` + `docs/perf/PERF-A-LOAD-TEST.md`.** `LoadTestRunner` — pure asyncio (no locust/k6/numpy), pure-Python `percentile()`; DB-direct mode (times the per-decision context-view query, the dominant cost — persona compute is sync sub-ms) + HTTP mode (httpx, already a dep). Scenarios: single / 10 / 100 concurrent / burst 10x10. **Measured meridian baseline (local → remote RDS, 2026-06-27):** single 563ms; 10-concurrent p50 1984 / p95 2034 / p99 2056; **100-concurrent p50 4554 / p95 6140 / p99 6140**; warm burst p95 323ms; 100% success. p95 exceeds the 5000ms target ONLY because the measurement pays internet RTT to the remote RDS — `EXPLAIN ANALYZE` actual is ~3ms; in-VPC co-location (HA-A) lands well under target. Exit 0 iff 100-concurrent p95 < 5000ms (CI gate).

**PERF-B — `scripts/perf/index_audit.py` + `docs/perf/PERF-B-DB-OPTIMIZATION.md`.** `IndexAuditor.audit()` — detects existing ordered-prefix index coverage, runs EXPLAIN ANALYZE (FORMAT JSON) on 3 hot views, flags genuine gaps, emits `CREATE INDEX CONCURRENTLY` DDL; `create_missing_indexes(dry_run=True)` (default prints; --create executes). **Findings:** hot tables already heavily indexed (42 indexes / 5 tables); the 3 hot views execute **2.5-3.2ms ALL via index scan** → latency is network RTT, not query plans. **Spec corrections:** `decision_outputs` has NO `persona_id` (key is `decision_id`) → recommend `(tenant_id, decision_id)`; `fact_nodes (application_id, tenant_id)` ALREADY exists (idx_fact_nodes_app) → auto-detected, not re-recommended. 4 marginal composite gaps recommended. Partition strategy for decision_trace/audit_log @ >10M rows (documented, not implemented — current ~142k/~263k rows far below trigger). Two self-caught bugs fixed: EXPLAIN FORMAT JSON returns text (json.loads), and the coverage check's set-subset branch was too loose (strict ordered-prefix only).

**PERF-C — `core/infra/cache.py` + `docs/perf/PERF-C-CACHING.md`.** `EvidenceCache` — in-memory TTL cache (default 300s, no Redis dep) keyed by `(tenant_id, application_id, document_index_updated_at)`; the doc-timestamp is the PRIMARY invalidation (changed doc → updated_at advances → new key → miss), TTL is the safety net, `invalidate(tenant, app)` for explicit eviction; asyncio-safe (lock), lazy eviction on read. RULE 11: `get()` returns `cache_hit`; `stats()` carries data_source + missing_inputs. **STANDALONE** — `evidence_facts()` is on the runner decision path (runner.py:340-362) so the cache is NOT wired (would risk 16/16); the exact wiring snippet + Redis upgrade path (same get/set/invalidate interface) are documented. High hit-rate expected on stable tenants (meridian docs never change); a hit replaces a ~140-560ms RDS round-trip with a microsecond dict lookup.

**Verification.** Load baseline ran (real p50/p95/p99 captured); EXPLAIN confirmed index usage; cache transitions (hit/miss/TTL-expiry/invalidate/singleton) all pass. No new Python failures. **16/16 by construction** (read-only/standalone). **All 125 build prompts complete.**


## Landing VideoSection (two-column fraud card) + frontend prod deploy — 2026-06-28

**Frontend/landing + ops (commit 77527fe) — NOT a decision-engine change.** Replaced the landing-page VideoSection with a two-column layout and shipped the frontend to prod ECS. No backend/rules/personas/catalogue touched (ARCHITECTURE.md unchanged by design).
...
## Customer-facing docs + blog site, landing polish, and product-card video demos — 2026-07-28 → 30

Public marketing surface built out on the `/` landing app (served by the `accord-frontend` ECS service via nginx + Vite `public/`). All frontend-only; nothing here touches the decision engine, personas, or the 16/16 eval. Deployed with `bash deploy.sh frontend` (Docker build → ECR → ECS force-new-deployment); verified live on the ALB each pass by diffing the served `dist/assets/index-*.js` hash and curling routes/assets.

- **Docs section** (`f3437dd`) — new `DocList` (card index) + `DocPost` (slug-routed article renderer, `DOCS` registry) under `/docs`, wired into `App.tsx`. Six guides: `getting-started`, `data-model`, `enterprise-security`, `soc2-ready`, `hmda-fair-lending`, `compliant-by-design`. Articles use raw `h2`/`ul`/`table` + amber "Planned Enhancements — Not Yet Implemented" callouts (honest shipped-vs-roadmap framing); early `Section`/`Fact` helper components are now unused but retained (`noUnusedLocals: false`). Requires `@tailwindcss/typography` (`prose` styling) — added to `package.json` + `index.css` `@plugin`. nginx SPA fallback tightened to `try_files $uri /index.html` so client-side `/docs` + `/blog` routes resolve.
- **Blog** (`f3437dd`) — `BlogList` + `BlogPost` under `/blog`; posts include `dti-trap-200-basis-points` and `seven-reasons-loans-get-blocked`. BlogPost media (screenshots + clips) live under untracked `frontend/public/blog/`.
- **Landing polish** (`f3437dd`) — `LandingNav` Docs/Blog tabs point to `/docs` + `/blog`; `ImpactSection` testimonial quote box removed (stats now full-width 4-up), which also dropped a fabricated design-partner quote; `SimulationSection` copy refreshed to real portfolio numbers; `Landing.tsx` robust scroll deep-linking (`?scroll=` / hash, retry loop).
