...
function RequestDocsModal({ loan, onClose, onDone }: {
  ...
  // Email-preview values — all derived from the form + auth/loan context.
  const borrowerFirstName = (loan.borrower?.name ?? '').trim().split(/\s+/)[0] || 'there'
  const underwriterName = effectiveUser?.name ?? 'your underwriter'
  const tenantName = tenant?.name ?? (loan as any).tenant_name ?? 'your lender'
          ...
          <div>
            ...
            <div className="mt-1 text-[12px] text-slate-500">
              {step === 1
                ...
                : <>Review the email {loan.borrower?.name ?? 'the borrower'} will receive before sending.</>}
            </div>
          </div>
