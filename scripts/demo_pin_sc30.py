...
"protected" in the demo video (pinned to summit rule v1, current is v3).
  ...
  python scripts/demo_pin_sc30.py          # pin to v1 (protected)
  python scripts/demo_pin_sc30.py --unpin  # restore (pinned_rule_version = NULL)

Reversible — APP-SC30-004 starts with pinned_rule_version=NULL, so --unpin
returns it exactly to its original state.
"""
import argparse
import asyncio
import os
...
async def main(unpin: bool) -> None:
    ...
    try:
        if unpin:
            await conn.execute(
                "UPDATE entity_states SET pinned_rule_version=NULL, pinned_at=NULL "
                "WHERE application_id=$1 AND tenant_id=$2", APP, TENANT)
            print(f"{APP} unpinned (pinned_rule_version = NULL).")
        else:
            ...
            await conn.execute(
                "UPDATE entity_states SET pinned_rule_version=$1, pinned_at=$2 "
                "WHERE application_id=$3 AND tenant_id=$4", v1, PINNED_AT, APP, TENANT)
            print(f"{APP} pinned to summit rule v1 ({v1}) at {PINNED_AT} — protected vs current v3.")
        row = await conn.fetchrow(
            "SELECT tr.version AS pinned_v, cur.version AS current_v, es.pinned_at "
        ...
        print(f"  now: pinned_v={row['pinned_v']} current_v={row['current_v']} pinned_at={row['pinned_at']}")
    finally:
        await conn.close()
